...
test('SpriteAnimator getSpritePosition will get the correct position of the step for a horizontal direction with wrapAfter', function (t) {
  ...
  t.equal(basicTest, '0px 0px', 'getSpritePosition produces correct position on basic test')
  t.equal(anotherStepTest, '-10px 0px', 'getSpritePosition produces correct position on another step test')
  t.equal(wrapStepTest, '-30px -10px', 'getSpritePosition produces correct position on another step test with row wrap')
  t.equal(biggerWrapStepTest, '-20px -20px', 'getSpritePosition produces correct position on another step test with row wrap')
  t.equal(extremeStepTest, '0px -2000px', 'getSpritePosition produces correct position on a extreme step test with row wrap')
  t.end()
})
...
test('SpriteAnimator getSpritePosition will get the correct position of the step for a vertical direction with wrapAfter', function (t) {
  ...
  t.equal(basicTest, '0px 0px', 'getSpritePosition produces correct position on basic test')
  t.equal(anotherStepTest, '0px -10px', 'getSpritePosition produces correct position on another step test')
  t.equal(wrapStepTest, '-10px -30px', 'getSpritePosition produces correct position on another step test with row wrap')
  t.equal(biggerWrapStepTest, '-20px -20px', 'getSpritePosition produces correct position on another step test with row wrap')
